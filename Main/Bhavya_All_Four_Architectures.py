...
PARSER_PROMPT = """You are a semantic classifier for debate motions.
Your goal is to map the motion to a high-level domain to determine which logic schemas to load.

Input Motion: "{motion}"

Task:
1. Identify the primary Domain from the following list: Economics, Politics and Governance, Law and Justice, Society and Culture, International Relations, Urban and Environment, Technology and AI, Education and Labor, Bioethics, Media (General).
...
SLOT_FILLER_PROMPT = """You are a logic engine. Your goal is to apply an abstract argument template to a specific motion.
...
1. Introduction (approx. 50 words): Quick reaffirmation of your team's stance.
2. Rebuttal (approx. 400 words): Systematically dismantle the Opposition's main arguments. Address their strongest points directly.
3. Defense (approx. 200 words): Rebuild any of your partner's arguments that were attacked.
4. Extension (approx. 600 words): Introduce one new, significant argument that adds a fresh dimension to the debate. Use Premise-Mechanism-Impact structure with depth.
5. Conclusion (approx. 50 words): Brief summary.""",
...
1. Introduction (approx. 50 words): Restate your team's position.
2. Rebuttal (approx. 400 words): Attack the Proposition's core arguments systematically.
3. Defense (approx. 200 words): Rebuild your partner's case where it was challenged.
4. Extension (approx. 600 words): Present one substantial new argument using rigorous Premise-Mechanism-Impact structure.
5. Conclusion (approx. 50 words): Brief summary.""",
...
1. Introduction (approx. 100 words): Frame the debate - what were the key clashes?
2. Clash Analysis (approx. 900 words): Go through 3-4 major points of clash in the debate. For each:
    - State what both teams said
    - Explain why your team won that clash
    - Comparatively weigh the impacts
3. Rebuilding (approx. 200 words): Address any final attacks from Opposition and reinforce your team's strongest arguments.
4. Conclusion (approx. 100 words): Synthesize why, on balance, your team has won.""",
...
1. Introduction (approx. 100 words): Frame the debate - identify the critical clashes.
2. Clash Analysis (approx. 900 words): Systematically go through 3-4 key clashes. For each:
    - Summarize both sides' positions
    - Prove why Opposition won that clash
    - Weigh impacts comparatively
3. Rebuilding (approx. 200 words): Counter final Proposition attacks and reinforce your strongest points.
4. Conclusion (approx. 100 words): Explain why, overall, Opposition has proven the superior case."""
}
...
class DebateSpeaker:
    ...
    def generate_speech(self, motion, teammate_speech, opponent_speeches):
        ...
        elif self.architecture == "detailed_prompts":
            return self._generate_detailed_prompts(motion, teammate_speech, opponent_speeches)
    ...
    def _generate_detailed_prompts(self, motion, teammate_speech, opponent_speeches):
        prompt = DETAILED_PROMPTS[self.speaker_position]
        context = f"Motion: {motion}\n\n"
    ...
    def _generate_schema_guided(self, motion, teammate_speech, opponent_speeches):
        clustered_threats = ""
        ...
        if self.speaker_number in [1, 2]:
            parser_prompt = PARSER_PROMPT.format(motion=motion)
            ...
            for schema in schemas:
                filler_prompt = SLOT_FILLER_PROMPT.format(
                    motion=motion,
...
1. Argumentation and Analysis (45%)
...
Examples:
  ...
  python crossover_debate.py -t 2 -pm 4o-mini -om 4o-mini -pa baseline -oa enhanced
    ...
    parser.add_argument('-pa', '--prop-arch', type=str, default='baseline',
                        choices=['baseline', 'detailed_prompts', 'enhanced', 'schema_guided'],
